...
path <- "/Volumes/RECHERCHE/FAC/FBM/DMF/pvonaesc/default/D2c/Jazmine Mote/16S Results/Poop_clean"
# Control files
list.files(path)
```
...
length(which(otu_singleton_rowsums <= 50)) # 2176
```
...
write.table(track, "../Poop_clean/new results/read_retention_table_PASTOBIOME_ORIGINAL_NEW.txt", quote=F, row.names=T, col.names=T, sep="\t")
...
taxa <- taxa %>%
  as.data.frame() %>%
  mutate(ASV = paste("ASV_original_", seq(1:nrow(taxa)), sep="")) %>%
...
# Save taxonomy table
write.table(taxa, "../Poop_clean/new results/taxonomy_table_PASTOBIOME_ORIGINAL_NEW.txt", sep="\t", quote=F, row.names=T, col.names=T)
...
# Save sequence table
write.table(otu, "../Poop_clean/new results/abundance_table_PASTOBIOME_ORIGINAL_NEW.txt", sep="\t", quote=F, row.names=T, col.names=T)
