...
setwd("/Volumes/RECHERCHE/FAC/FBM/DMF/pvonaesc/default/D2c/Jazmine Mote/16S Results")

# Save path to fastq file folder after unzipping
path <- "/Volumes/RECHERCHE/FAC/FBM/DMF/pvonaesc/default/D2c/Jazmine Mote/16S Results/SICs_clean"
# Control files
list.files(path)
```
...
Error in dada(derepRs, err = errR, multithread = TRUE) : 
  Invalid derep$uniques vector. Sequences must be made up only of A/C/G/T.
  when including S213 - will try without it 
```{r}
...
write.table(track, "../SICs_clean/new results/read_retention_table_PASTOBIOME_STABILITY_NEW.txt", quote=F, row.names=T, col.names=T, sep="\t")
...
taxa <- taxa %>%
  as.data.frame() %>%
  mutate(ASV = paste("ASV_stability_", seq(1:nrow(taxa)), sep="")) %>%
...
# Save taxonomy table
write.table(taxa, "../SICs_clean/new results/taxonomy_table_PASTOBIOME_STABILITY_NEW.txt", sep="\t", quote=F, row.names=T, col.names=T)
...
# Save sequence table
write.table(otu, "../SICs_clean/new results/abundance_table_PASTOBIOME_STABILITY_NEW.txt", sep="\t", quote=F, row.names=T, col.names=T)
