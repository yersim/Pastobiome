...
colnames(df_seq2) <- c("#Samples", "mean", "max", "min", "sd", "sum", "<10'000")
rownames(df_seq2) <- country_list$Country
for (j in 1:3){
      df_seq2[j,1] <- length(which(sample_sums_df2$Country==country_list[j,]))
      df_seq2[j,2] <- mean(sample_sums(subset_samples(physeqMI_filt, Country==country_list[j,])))
      df_seq2[j,3] <-  max(sample_sums(subset_samples(physeqMI_filt, Country==country_list[j,])))
      df_seq2[j,4] <- min(sample_sums(subset_samples(physeqMI_filt, Country==country_list[j,])))
      df_seq2[j,5] <- sd(sample_sums(subset_samples(physeqMI_filt, Country==country_list[j,])))
      df_seq2[j,6] <- sum(sample_sums(subset_samples(physeqMI_filt, Country==country_list[j,])))
...
df_seq2 <- as.data.frame(df_seq2) %>%
  mutate(n_taxa = "")
df_seq2[1,10] <- as.numeric(ntaxa(physeq_ethiopiaMI))
df_seq2[2,10] <- as.numeric(ntaxa(physeq_madagascarMI))
df_seq2[3,10] <- as.numeric(ntaxa(physeq_CARMI))
# Save table with sequencing info after filtering
write.csv(df_seq2, "MI_sequencing_info_postfilt.csv")
```
Subset by country
```{r}
...
pal_alpha <-palbrew_all[c(60,69,62)]
...
```{r}
meta_pcoa = as.matrix(sample_data(physeq_prl_sp))
meta_pcoa <- as.data.frame(meta_pcoa)
# distance matrix
distance_pcoa_wuf <- distSP.wuf
...
pdf("PosterFam_select_Afribiota_Log.pdf", width = 18, height = 8)
